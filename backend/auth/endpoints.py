...
async def get_users(
        ...
        token: str = Depends(Auth.decode_token)) -> List[Dict[str, UserList]]:
    if token:
...
async def create_user(
        body: UserCreate,
        session: AsyncSession = Depends(get_async_session)) -> UserCreateResponse:
...
async def update_user(
    ...
    if token:
        ...
        if updated_user_params == {}:
            raise HTTPException(status_code=422, detail='You must specify at least one user update option')
        user = await UserSession(session).update_user(user_id, updated_user_params)
        if user is None:
            raise HTTPException(status_code=404, detail=f'User with id {user_id} not found.')
        return UserUpdate(username=user.username, phone=user.phone, email=user.email)
...
async def refresh_token(data: RefreshTokenBody = Depends()) -> RefreshToken:
    if getattr(data, 'refresh'):
        ...
        if refresh:
            ...
            return decode_token
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Incorrect refresh token')
